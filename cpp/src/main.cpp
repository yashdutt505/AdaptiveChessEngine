...
void run_search(ace::Position position,ace::TranspositionTable& table,GoParameters parameters,int move_overhead,std::atomic<bool>& stop){
    ...
    std::uint64_t total_nodes=0;table.new_search();
    for(int depth=1;depth<=parameters.depth&&!stop.load(std::memory_order_relaxed);++depth){
        ...
        ace::Searcher searcher(&table);auto result=searcher.search(position,depth,limits);total_nodes+=result.nodes;
...
class SearchWorker {
    struct Task {ace::Position position;ace::TranspositionTable* table;GoParameters parameters;int overhead;std::atomic<bool>* stop;};
#ifdef _WIN32
    HANDLE handle_=nullptr;
    static DWORD WINAPI entry(void* raw){std::unique_ptr<Task> task(static_cast<Task*>(raw));run_search(std::move(task->position),*task->table,task->parameters,task->overhead,*task->stop);return 0;}
#else
    std::thread thread_;
#endif
public:
    ...
    void start(ace::Position position,ace::TranspositionTable& table,GoParameters parameters,int overhead,std::atomic<bool>& stop){
        auto* task=new Task{std::move(position),&table,parameters,overhead,&stop};
#ifdef _WIN32
...
#else
        thread_=std::thread([task](){std::unique_ptr<Task> owned(task);run_search(std::move(owned->position),*owned->table,owned->parameters,owned->overhead,*owned->stop);});
#endif
    }
    void join(){
#ifdef _WIN32
...
int main(){
    ...
    int move_overhead=50;std::atomic<bool> stop{false};SearchWorker worker;std::string line;
    ...
    while(std::getline(std::cin,line)){
        try{
            ...
            if(command=="uci")std::cout<<"id name Adaptive Chess Engine C++\nid author Yash Dutt\noption name Hash type spin default 64 min 1 max 1024\noption name Clear Hash type button\noption name Move Overhead type spin default 50 min 0 max 5000\nuciok\n"<<std::flush;
                ...
                if(name=="Hash"&&!value.empty())table.resize(static_cast<std::size_t>(std::stoul(value)));else if(name=="Clear Hash")table.clear();else if(name=="Move Overhead"&&!value.empty())move_overhead=std::max(0,std::min(std::stoi(value),5000));
                ...
                worker.start(snapshot,table,parameters,move_overhead,stop);
            }
