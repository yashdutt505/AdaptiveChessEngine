...
struct SearchResult { Move best_move=0; int score=0; int depth=0; std::uint64_t nodes=0; std::vector<Move> pv; long long time_ms=0; };
...
public:
    ...
    SearchResult search(Position& p,int depth) {
        nodes=0; start_=std::chrono::steady_clock::now(); std::vector<Move> pv;
        const int score=negamax(p,depth,-Infinity,Infinity,0,pv);
        return {pv.empty()?0:pv[0],score,depth,nodes,pv,std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::steady_clock::now()-start_).count()};
    }
private:
    ...
    int quiescence(Position& p,int alpha,int beta,int ply) {
        ++nodes; if(draw(p)) return 0; const bool checked=in_check(p,p.side_to_move);
        ...
        for(Move move:ordered(p,std::move(moves),0)) { p.make_move(move); const int score=-quiescence(p,-beta,-alpha,ply+1); p.unmake_move(); if(score>=beta)return beta; if(score>alpha)alpha=score; }
        return alpha;
    }
    int negamax(Position& p,int depth,int alpha,int beta,int ply,std::vector<Move>& pv) {
        ++nodes; if(draw(p)) return 0; if(depth==0)return quiescence(p,alpha,beta,ply);
        const int original=alpha; Move tt_move=0;
        if(tt_) if(auto* e=tt_->probe(p.hash_key)) { tt_move=e->move; if(e->depth>=depth){int s=e->score; if(s>MateScore-128)s-=ply; if(s<-MateScore+128)s+=ply; if(e->bound==Exact)return s; if(e->bound==Lower)alpha=std::max(alpha,s);else beta=std::min(beta,s);if(alpha>=beta)return s;}}
        ...
        for(Move move:ordered(p,std::move(moves),tt_move)) {
            ...
            p.unmake_move(); if(score>best){best=score;best_move=move;best_child=child;} if(score>alpha)alpha=score;if(alpha>=beta)break;
        }
