...
int main(){
    ...
    ace::SearchHeuristics heuristics;std::vector<ace::Move> failed{failed_move};heuristics.record_cutoff(cutoff,4,2,0,1,previous,failed);assert(heuristics.killer_rank(cutoff,2)==2);assert(heuristics.history_score(cutoff,0)==16);assert(heuristics.is_countermove(cutoff,previous));assert(heuristics.history_score(failed_move,0)<0);
}
